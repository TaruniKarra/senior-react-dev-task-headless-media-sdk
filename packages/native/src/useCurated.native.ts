...
/** Identical contract to useCurated() in @headless-media/wrappers. */
export function useCurated() {
  ...
  const fetchPage = useCallback(async (p: number, reset: boolean) => {
    ...
    try {
      ...
      setHasMore(result.hasMore); setPage(p);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    } finally { setLoading(false); }
  }, [client]);

  useEffect(() => { void fetchPage(1, true); }, [fetchPage]);
  const loadMore = useCallback(() => { if (!loading && hasMore) void fetchPage(page + 1, false); }, [loading, hasMore, page, fetchPage]);
